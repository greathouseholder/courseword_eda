...
df_prop <- df_long %>%
  ...
  mutate(
    # Присваиваем имя группы для соответствия палитре
    color_group = as.character(Price_Group) 
  )

p_proportions <- ggplot(df_prop, aes(x = Price_Group, y = Prop, fill = color_group)) +
  # Столбчатая диаграмма
  geom_bar(stat = "identity", width = 0.5, show.legend = FALSE) +
  
  # Добавляем текстовые подписи с процентами над столбцами
  geom_text(aes(label = format_perc(Prop)), vjust = -0.5, size = 3.5, family = "serif") +
  ...
  labs(
    ...
    y = "Доля выбора формата «Только ИИ»"
  )

# Отображаем и сохраняем
print(p_proportions)
ggsave_temp("proportions.png", p_proportions)

# ==============================================================================
# 2. ГРАФИК С ДОВЕРИТЕЛЬНЫМИ ИНТЕРВАЛАМИ ДЛЯ "ТОЛЬКО ИИ"
# ==============================================================================

# Расчет точечных оценок и 95% доверительных интервалов
df_ci <- df_long %>%
  group_by(Price_Group) %>%
  summarise(
    Prop = mean(AI_Solo),
...
p_intervals <- ggplot(df_ci, aes(x = Price_Group, y = Prop, color = color_group)) +
  # Строим усы ДИ
  geom_errorbar(aes(ymin = Lower, ymax = Upper), width = 0.1, linewidth = 0.8, show.legend = FALSE) +
  
  # Добавляем среднее значение
  geom_point(size = 3, show.legend = FALSE) +
  geom_text(aes(label = format_perc(Prop)), hjust = -0.2, vjust = -0.5, size = 3.5, family = "serif", color = "black") +
...
# Отображаем и сохраняем
print(p_intervals)
ggsave_temp("confidence_intervals.png", p_intervals)

# ==============================================================================
# 3. ГИСТОГРАММА РАСПРЕДЕЛЕНИЯ ИНДИВИДУАЛЬНЫХ ПРЕДПОЧТЕНИЙ
# ==============================================================================

# Рассчитываем индивидуальную склонность каждого респондента выбирать соло ИИ
df_individual <- df_long %>%
  group_by(Respondent_ID) %>%
  summarise(
    AI_Rate = mean(AI_Solo),
    .groups = 'drop'
  )

p_histogram <- ggplot(df_individual, aes(x = AI_Rate)) +
  # Строим гистограмму
  geom_histogram(
    aes(fill = "ИИ"),
    binwidth = 0.1, 
  ...
  # Форматируем ось X в проценты с шагом 10
  scale_x_continuous(
    labels = percent_format(accuracy = 1),
  ...
  labs(
    x = "Доля выбора «Только ИИ» респондентом",
    y = "Количество респондентов"
  )

# Отображаем и сохраняем
print(p_histogram)
ggsave_temp("individual_preferences.png", p_histogram)